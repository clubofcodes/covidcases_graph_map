...
import { LineChart, Line, CartesianGrid, XAxis, YAxis, Tooltip, ResponsiveContainer, Legend, Scatter, Area } from 'recharts';
        ...
        apiError ? <div> {apiError} </div> :
            casesData.length ?
                <ResponsiveContainer width="100%" aspect={2.5}>
                    <LineChart data={casesData.slice(0, 15)} width={500} height={300} margin={{ top: 5, right: 30, bottom: 5, left: 40 }} >
                        <CartesianGrid stroke="white" strokeDasharray="3 3" />
                        <Line type="monotone" dataKey="cases" stroke="red" dot={{ r: 5 }} activeDot={{ stroke: 'white', strokeWidth: 2, r: 8 }} strokeWidth={2} />
                        <XAxis dataKey="country" interval={'preserveStartEnd'} minTickGap={0} tickSize={15} allowDataOverflow={true} />
                        <YAxis />
                        <Legend />
